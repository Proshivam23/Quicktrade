    ...
    async function fetchData() {
      try {
        const response = await axios.get(
          `http://localhost:8081/page/${prodid}`
        );
        ...
        <div className="p-4 flex justify-center items-center">
          <div className="w-1/2">
            ...
            <Carousel
              ...
              infiniteLoop={true}
              dynamicHeight={false}
              width="100%" // Adjust the carousel width
            >
              <div className="w-96 h-96 relative">
                <img
                  src={`http://localhost:3000/${data[0].image1}`}
                  alt="Slide 1"
                  className="w-fit h-fit object-contain" // Adjust slide width and maintain aspect ratio
                />
          ...
          <div className="w-1/2">
            <div class="w-full m-6 p-6 pt-4 pb-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              <h2 className="font-montserrat-semibold text-semibold border-b-2 pb-2" w-full>{data[0].title}</h2>
              <h1 className="text-xl font-bold">₹.{data[0].price}</h1>
              <h5 className="font-montserrat font-semibold w-fit">Description:</h5>
              <p className="text-muted border-2 rounded-md">{data[0].description}</p>
              <button
                type="button"
              ...
              <button
                ...
                value={prodid}
                type="button"
                className="text-white ml-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
              >
                <svg
              ...
              <div className=" mt-4">
                <p className="font-montserrat font-semibold">Seller details:</p>
                <p className="text-muted font-montserrat">Name: {data[0].username} </p>
                <p className="text-muted font-montserrat">Address: {data[0].location}</p>
                <button
                  value={data[0].seller_id}
                  onClick={handleClick}
                  type="button"
